...
int dnslookup_multiIP(const char* hostname, queue* IPlist){
    ...
    if(!queue_is_empty(IPlist))
    {
		printf("IPlist should be empty\n");
		return UTIL_FAILURE;
	}
	...
	if(result->ai_addr->sa_family == AF_INET){
	    ...
	    // malloc an ipv4str
	    char *ipv4str = (char *)malloc(sizeof(char) * INET_ADDRSTRLEN);
	    
	    // check for failed malloc
	    if(ipv4str == NULL)
	    {
			fprintf(stderr, "Heap out of memory; malloc failed in util.c\n");
			return UTIL_FAILURE;
		}
		...
		// push string to IP string queue if not full
		if(queue_is_full(IPlist))
		{
			printf("IPlist queue is full...cannot push\n");
			// free the last ip string and return
			free(ipv4str);
